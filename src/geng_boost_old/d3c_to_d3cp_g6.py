...
import os                   # ファイル・ディレクトリ操作 / File and directory handling

# 頂点数 n をユーザー入力から取得
# Prompt user for number of vertices
n = int(input("Enter the number of vertices (>= 4): "))

input_dir = f"d3c/n{n}"       # 入力ディレクトリ / Input directory
output_dir = f"d3cp/n{n}"     # 出力ディレクトリ / Output directory

# 出力ディレクトリがなければ作成 / Create output directory if not exists
os.makedirs(output_dir, exist_ok=True)

# 入力ディレクトリ内のすべての .g6 ファイルをソート順に処理
# Process all .g6 files in input directory (sorted by name)
for filename in sorted(os.listdir(input_dir)):
    ...
    print(f"Filtering planar graphs in: {input_path}")
    ...
    with open(input_path, mode="rt") as f_in, open(output_path, mode="wt") as f_out:
        ...
        # C++ プログラムから出力を受け取り、ファイルに書き出す
        # Read planar graph6 strings from C++ and write to output file
